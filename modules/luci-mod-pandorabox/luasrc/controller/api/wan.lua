...
function _getWanProto ()
	if not nixio.fs.access('/usr/bin/detect_wan') then
		return code.code40003('Miss detect_wan', guide_options)
	end

	local rv = PBCommand.get('/usr/bin/detect_wan >/dev/null 2>/dev/null;echo $?')
